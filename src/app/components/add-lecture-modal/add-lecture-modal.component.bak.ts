...
export class AddLectureModalComponent implements OnInit {
  ...
  dayStartHour = 9;
  startDatetimeMin = '2020-03-28T08:00:00.429Z';
  endDatetimeMax = '2020-03-28T17:00:00.429Z';
  lectureData;
  user: any;
  constructor(
  ...
  submitForm(form: FormGroup) {
    ...
    const startHr = getHours(new Date(form.value.startTime));
    let startMn = getMinutes(new Date(form.value.startTime)).toString();
    ...
    const endHr = getHours(new Date(form.value.endTime));
    let endMn = getMinutes(new Date(form.value.endTime)).toString();
